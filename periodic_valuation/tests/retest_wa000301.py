...
on the actual documents.
  ...
  bench --site <site> execute periodic_valuation.tests.retest_wa000301.run --kwargs '{"commit": true}'
...
def item(code):
	if not frappe.db.exists("Item", code):
		...
		frappe.get_doc({
			...
			"stock_uom": uom, "is_stock_item": 1, "valuation_method": "Periodic Moving Average",
